...
- 输出必须包含延迟秒与实时标记; 若延迟秒 > 默认最大延迟(2分钟×60), 将决策降级为观望
- 市场休市或数据源延迟时必须标注非实时, 并限制仓位不超过5%
- 默认最大延迟分钟为2, 可按场景调整但需明示

## 输出格式
- 建议输出Markdown摘要, 字段定义如下
- Markdown摘要:
  - 标题: "{{symbol}} @ {{timeframe}} — 决策摘要"
  - 要点: 决策, 仓位, 风控, 关键法则证据与评分
  - 风险披露与合规提示
- 必含元数据:
  - 数据源: "{{数据源}}"
  - 当前股价: "{{当前股价}}"
...
- 标题: "AAPL @ 1d — 决策摘要"
- 要点:
  - 决策: 买入; 仓位: 15.0%; 入场: 198.5–200.0
  - 风控: 止损2.0%; 止盈4.0%; 移动止损2.5%
  - 关键证据: 量增价涨评分0.78, 成交量1.62x 20日均量
  - 风险披露: 趋势不确定, 事件驱动风险; 合规提示已确认
- 元数据:
  - 数据源: YahooFinance
  - 当前时间: 2025-12-02T14:31:00Z
  - 报价时间: 2025-12-02T14:30:00Z
  - 延迟秒: 60
  - 实时: true
- 断言:
  - 成交量放大断言通过
